...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    // Combined password strength check
    ...
    try {
      ...
      await updateUser({ displayName: fullName, photoURL }).catch((err) => {
      ...
      const response = await fetch('https://assignment-10-grapes-server.vercel.app/users', {
        method: 'POST',
      ...
      setUser({ ...currentUser, displayName: fullName, photoURL });
        ...
        <div className="w-full lg:w-1/2">
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <input type="url" name="photoURL" id="photoURL" required placeholder="Profile Picture URL"
                className="w-full px-4 py-2 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400" />
            </div>
            ...
            <button type="submit"
              className="w-full py-2 px-4 rounded-full text-white font-semibold bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-indigo-600 hover:to-blue-600 transition"
              disabled={loading}>
